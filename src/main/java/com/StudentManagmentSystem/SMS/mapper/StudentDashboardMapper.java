...
public class StudentDashboardMapper {
    ...
    Double gpa,List<Enrollment> enrollments){
         ...
         studentDashboard.setSemesterId(enrollments.get(0).getCourseOffering().getSection().getSemester().getId());
         studentDashboard.setSemesterName(enrollments.get(0).getCourseOffering().getSection().getSemester().getSemesterName());
